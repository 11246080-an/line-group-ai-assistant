...
def _judge_same_topic_with_llm(
    ...
    result = _call_small_json_model(
        ...
        user_prompt=(
            ...
            "如果最近對話是在累積同一趟行程想去的多個地點，"
            "例如『我想去101』『還想去松菸』『再去西門町』『幫我排行程』，"
            "這是在列同一趟行程的候選景點，same_topic=true。"
            "但如果最近對話是一次獨立資訊查詢，新訊息又改問另一個地點的同類查詢，"
            "例如『三峽附近有什麼景點』後接『宜蘭有什麼景點可以玩』，"
            "或『北車附近有什麼吃的』後接『台中有什麼吃的』，"
            "這是新的查詢主題，same_topic=false。"
            "只有在新訊息明顯改成另一個獨立任務、另一個日期安排、"
            "或明確表示不再討論前一件事時，same_topic=false。"
            "如果是『選項累積/補充條件』而不確定，優先 same_topic=true；"
            "如果是『地點已更換的獨立查詢』而不確定，優先 same_topic=false。"
        ),
    )
