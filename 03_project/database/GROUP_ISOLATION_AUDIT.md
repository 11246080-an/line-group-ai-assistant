...
| API 查詢紀錄 | `db.py: api_query_cache` collection | ✅ 設計上刻意共用 + 可追蹤 | 見第三節說明 |
...
`api_query_cache` 這次補上一個**選填**的 `line_group_id` 欄位（`save_api_query_cache(..., line_group_id=...)`），記錄是哪個群組觸發了這次外部查詢，方便之後追蹤/除錯。

**這裡刻意沒有做成強制隔離**，原因：外部 API 回傳的是天氣、餐廳、電影場次這類公開資訊本身，不是群組的私有對話內容。群組 A 問「台中火鍋」和群組 B 問「台中火鍋」，兩邊拿到同一份餐廳搜尋結果並沒有隱私問題，共用快取反而比較有效率，不用重複打外部 API 額度。

如果之後某個查詢類型的結果本質上跟群組私有資訊綁在一起（例如把該群組的偏好也編進了查詢條件），處理方式是呼叫端自己把 `line_group_id` 併進 `query_key`（例如 `f"{line_group_id}:{query_key}"`），這樣同一個 `query_type` 底下不同群組自然會落在不同的快取鍵，不需要改 `db.py`。
